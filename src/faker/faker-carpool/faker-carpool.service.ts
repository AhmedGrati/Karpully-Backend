...
export class FakerCarpoolService {
    ...
    async seed() {
        ...
        if(allCarpools.length < seedNumber) {
            return await Array.from({length: seedNumber}).map<Promise<Carpool>>(async () => {
                const departureCityId: number = allCities[0].id;
                const destinationCityId: number = allCities[1].id;
                const owner = allUsers[0];
                const hasSmokePermission: boolean = faker.random.arrayElement([true, false]); 
                ...
                const carpool: CreateCarpoolInput = {nbrOfAvailablePlaces, departureCityId, description, destinationCityId, hasSmokePermission, departureDate};
