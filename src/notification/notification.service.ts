...
export class NotificationService {
  ...
  publishNotification(notification: Notification) {
    this.pubSub.publish(BASE_TOPIC_NAME+notification.receiver.id, {notification})
  }

  async findAllByUserId(userId: number, paginationInput: PaginationInput):Promise<PaginatedNotification> {
    const queryBuilder = this.notificationRepository.createQueryBuilder("notification")
    .where('notification.receiver.id = :userId', {userId})
    .leftJoinAndSelect("notification.receiver","user");
    return await Pagination.paginateQueryBuilder<Notification>(queryBuilder, paginationInput);
